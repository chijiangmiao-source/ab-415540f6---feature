...
- 规则前提带**极性**:肯定前提必须成立,否定前提(“仅当某事实/结论**不成立**
  时才放行”的例外条件)必须缺席。页面录入时用裸标识表示肯定、`!B`(或 `~B`)
  表示否定;JSON 中否定前提写作 `{"node":"B","polarity":"negative"}`。
- 结论有效 ⟺ 它处于当前有效事实上规则**按依赖分层的最小不动点**中。否定边
  一定指向更低的层,缺席判断只针对已定真值,因此既不会提前“猜缺”,也不会震荡。
- 任何**含否定边的依赖环**都被拒绝(没有稳定真值);仅含肯定边的环仍允许登记,
  但在没有落地事实支撑时不会凭空有效,落地支撑被撤回后随之坍塌。
- 每次规则触发都把**完整(带极性)前提集合**存入 `supports` 表;每项依据可看到
  实际满足的肯定项,以及“缺席已确认”的否定项。
- **断言**原先缺席的阻断事实 → 依赖该缺席条件的结论及其下游在同一事务内失效;
  **撤回**阻断事实后,仍满足其他前提的结论恢复,并与原有的正向支持并存而不被
  误撤。失效/恢复都沿 `rule_premises` 反向索引传播。
- 撤回事实 → 经 `rule_premises` 反向索引找到受影响结论子图 → 在子图上
  重算最小不动点 → 同一事务提交节点状态、支持状态、裁决与传播链。
- 结论只有在**没有任何完整支持**时才失效。
- 重复撤回已失效事实:稳定回放首次撤回时记录的裁决(`replayed: true`);
  对已断言事实重复断言同样回放,不产生新状态。
- 未知事实、引用不存在节点的规则、自我支持闭环(`A ⇒ A`)均被拒绝,
  且事务回滚,规程不被污染。批量规则可声明多节点循环,整批原子生效。
- 全部状态存于 SQLite,重启后结论与依据状态原样保留;旧库自动补齐极性列。
